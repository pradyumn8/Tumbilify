...
import { colorSchemes, dummyThumbnails, thumbnailStyles, type AspectRatio, type IThumbnail, type ThumbnailStyle } from '../assets/assets';
...
const Generate = () => {
    ...
    const [addtionalDetails, setAddtionalDetails] = useState('');
    ...
    const fetchThumbnail = async () => {
        if(id){
            ...
            setAddtionalDetails(thumbnail.user_prompt || '');
            setTitle(thumbnail.title || '');
            setAspectRatio(thumbnail.aspect_ratio as AspectRatio);
            setColorSchemeId(thumbnail.color_scheme_id || colorSchemes[0].id);
            setLoading(false)
        }
    }
    useEffect(() => {
                            ...
                            <div className='p-6 rounded-2xl bg-white/8 border border-white/12 shadow-xl space-y-6'>
                                ...
                                <div className='space-y-5'>
                                    ...
                                    <div className='space-y-2'>
                                        <label className='block text-sm font-medium text-zinc-300'>Additional Details<span className='text-zinc-400 text-xs'> (optional)</span></label>
                                        <textarea value={addtionalDetails} onChange={(e)=>setAddtionalDetails(e.target.value)} rows={3} placeholder='Add any specific elements, mood, or stile preferences...' className='w-full px-4 py-3 rounded-lg border border-white/100 bg-white/6 text-zinc-100 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none'></textarea>
                                    </div>
                                </div>
                                {/* Button */}
                                {!id && (
